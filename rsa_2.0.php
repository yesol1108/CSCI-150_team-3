<?php
	...
	function get_random_prime( $val = 128 ) // getting a random prime number.. gmp doesn't have a function for this.. so..
	{
		$seed = gmp_random_bits( $val ); // get a random number
		$prime = gmp_nextprime( $seed ); // get the next prima number
		return $prime;
	}
	function rsa_encrypt($message, $public_key_d, $public_key_n) // this is easy
	{
		$message = join(array_map(function ($n) { return sprintf('%03d', $n); },
                          unpack('C*', $message)));
	...
	function rsa_decrypt($value, $private_key_e, $public_key_n) // this one too
	{
		...
		$resp = join(array_map('chr', str_split($resp, 3)));
